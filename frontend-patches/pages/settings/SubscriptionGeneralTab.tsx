import { Alert, Input, InputNumber, Switch, Tabs, message } from 'antd';
...
export default function SubscriptionGeneralTab({ allSetting, updateSetting }: SubscriptionGeneralTabProps) {
  ...
  // moded by saeson: 订阅服务与 Clash / Mihomo 订阅相互绑定。
  // 仅开「启用订阅服务」而关闭 Clash/Mihomo 时，/clash/ 端点不会注册 → 客户端导入 404。
  // 因此二者保持一致：要么都开，要么都关。
  const handleSubEnableChange = (v: boolean) => {
    updateSetting({ subEnable: v, subClashEnable: v });
  };
  const handleSubClashChange = (v: boolean) => {
    if (v) {
      updateSetting({ subClashEnable: true, subEnable: true });
    } else {
      updateSetting({ subClashEnable: false, subEnable: false });
      message.warning('已关闭 Clash / Mihomo 订阅，订阅服务将一并关闭，/clash/ 与 /sub/ 端点均不可用。');
    }
  };
    ...
    <Tabs defaultActiveKey="1" items={[
      {
        ...
        children: (
          <>
            {allSetting.subEnable && !allSetting.subClashEnable && (
              <Alert
                type="warning"
                showIcon
                style={{ marginBottom: 16 }}
                message="Clash / Mihomo 订阅未开启"
                description="已开启「启用订阅服务」但未开启「Clash / Mihomo 订阅」：订阅服务的 /clash/ 端点不会注册，客户端通过该端点导入会返回 404。建议同时开启「Clash / Mihomo 订阅」开关（开启订阅服务时会自动一并开启）。"
              />
            )}
            <SettingListItem paddings="small" title={t('pages.settings.subEnable')} description={t('pages.settings.subEnableDesc')}>
              <Switch checked={allSetting.subEnable} onChange={handleSubEnableChange} />
            </SettingListItem>
            ...
            <SettingListItem paddings="small" title={t('pages.settings.subClashEnableTitle')}>
              <Switch checked={allSetting.subClashEnable} onChange={handleSubClashChange} />
            </SettingListItem>
