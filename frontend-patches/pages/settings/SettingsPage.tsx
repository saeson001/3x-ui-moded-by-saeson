...
// moded by saeson: 与防火墙模块对齐的 JSON 请求头（上游 axios-init.ts 默认表单编码）
...
export default function SettingsPage() {
  ...
  // moded by saeson: 保存订阅设置后，若防火墙已启用则自动同步放行订阅端口(默认 2096)。
  // 上游 defaultValueMap 中 subPort 默认为 2096，若用户从未保存过订阅设置，settings 表里
  // 就没有 subPort 键 —— 防火墙模块(已修复)会自动回退到该默认值放行。这里仅确保规则随保存刷新。
  async function syncFirewallAfterSave() {
    if (!activeSlug.startsWith('subscription')) return;
    try {
      const status = await HttpUtil.get<{ active?: boolean }>('/panel/api/firewall/status');
      if (!status?.active) return;
      await HttpUtil.post('/panel/api/firewall/sync', {}, JSON_HEADERS);
    } catch {
      // 防火墙模块不可用时静默跳过，不影响设置保存
    }
  }
