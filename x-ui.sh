...
check_config() {
    ...
    local has_default_cred=$(echo "$info" | grep -i 'hasDefaultCredential' | awk -F': ' '{print $2}' | tr -d '[:space:]')
    
    # 从数据库读取用户名
    if [[ -f /etc/x-ui/x-ui.db ]]; then
        admin_username=$(sqlite3 /etc/x-ui/x-ui.db "SELECT username FROM users ORDER BY id LIMIT 1;" 2>/dev/null || echo "")
    fi
    if [[ -z "$admin_username" && -f ${xui_folder}/x-ui.db ]]; then
        admin_username=$(sqlite3 ${xui_folder}/x-ui.db "SELECT username FROM users ORDER BY id LIMIT 1;" 2>/dev/null || echo "")
    fi
    
    if [[ "$has_default_cred" == "true" ]]; then
    ...
    if [[ -z "$server_ip" ]]; then
        echo -e "${yellow}无法自动获取服务器公网 IP，请手动输入。${plain}"
    ...
    echo ""
    echo -e "${blue}==================== 面板信息 ====================${plain}"
    echo -e "  ${green}访问地址:${plain}   http://${server_ip}:${existing_port}${existing_webBasePath}"
    echo -e "  ${green}管理员账号:${plain} ${admin_username:-admin}"
    echo -e "  ${green}管理员密码:${plain} ${admin_password}"
    echo -e "  ${green}面板端口:${plain}   ${existing_port}"
    echo -e "  ${green}基础路径:${plain}   ${existing_webBasePath}"
    echo -e "${blue}================================================${plain}"
    ...
    if [[ -n "$existing_cert" ]]; then
        ...
        if [[ "$domain" =~ ^[a-zA-Z0-9.-]+\.[a-zA-Z]{2,}$ ]]; then
            echo -e "${green}HTTPS 访问地址: https://${domain}:${existing_port}${existing_webBasePath}${plain}"
        else
            echo -e "${green}HTTPS 访问地址: https://${server_ip}:${existing_port}${existing_webBasePath}${plain}"
        fi
        if [[ -n "$cert_sans" && $(echo "$cert_sans" | wc -l) -gt 1 ]]; then
            echo -e "${yellow}证书同时覆盖域名:${plain} $(echo "$cert_sans" | grep -vx "$domain" | tr '\n' ' ')"
        fi
    else
        echo -e "${red}⚠ 警告: 未配置 SSL 证书！${plain}"
        echo -e "${yellow}可通过主菜单选项 20 申请 Let's Encrypt 证书（约 6 天有效期，自动续期）。${plain}"
        read -rp "是否立即为 IP 地址申请 SSL 证书? [y/N]: " gen_ssl
        if [[ "$gen_ssl" == "y" || "$gen_ssl" == "Y" ]]; then
            ...
            if [[ $? -eq 0 ]]; then
                ...
                LOGE "IP 证书申请失败。"
                echo -e "${yellow}可稍后通过主菜单选项 20 (SSL 证书管理) 重试。${plain}"
                start 0 > /dev/null 2>&1
            fi
        else
            echo -e "${yellow}HTTP 访问地址: http://${server_ip}:${existing_port}${existing_webBasePath}${plain}"
            echo -e "${yellow}为安全起见，建议通过主菜单选项 20 配置 SSL 证书。${plain}"
        fi
    fi

    # --- moded by saeson: Clash Link 信息 ---
    local clash_dir="/etc/x-ui/clash-configs"
    if [[ -d "$clash_dir" ]]; then
        echo ""
        echo -e "${blue}────────────────────────────────────────────${plain}"
        echo -e "${green}  Clash Link 订阅 (moded by saeson)${plain}"
        echo -e "${blue}────────────────────────────────────────────${plain}"
        echo -e "  API 地址:   ${green}http://${server_ip}:${existing_port}${plain}"
        echo -e ""
        echo -e "  生成配置:   POST /panel/api/clash-link/generate"
        echo -e "  查看列表:   GET  /panel/api/clash-link/list"
        echo -e "  备份导出:   GET  /panel/api/clash-link/backup"
        echo -e "  订阅链接:   /d/{token}"
        echo -e ""
        echo -e "  使用示例:"
        echo -e "  curl -X POST http://${server_ip}:${existing_port}/panel/api/clash-link/generate \\"
        echo -e "    -H 'Cookie: <你的会话Cookie>' \\"
        echo -e "    -d '{\"inbound_ids\":[1],\"config_name\":\"我的节点\"}'"
        echo -e "${blue}────────────────────────────────────────────${plain}"
    fi
}
