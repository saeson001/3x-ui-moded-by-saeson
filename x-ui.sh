...
check_config() {
    ...
    # --- moded by saeson: Clash Link info ---
    local clash_dir="/etc/x-ui/clash-configs"
    if [[ -d "$clash_dir" ]]; then
        echo ""
        echo -e "${blue}────────────────────────────────────────────${plain}"
        echo -e "${green}  Clash Link Subscription (moded by saeson)${plain}"
        echo -e "${blue}────────────────────────────────────────────${plain}"
        echo -e "  API Base:   ${green}http://${server_ip}:${existing_port}${plain}"
        echo -e ""
        echo -e "  Generate:   POST /panel/api/clash-link/generate"
        echo -e "  List:       GET  /panel/api/clash-link/list"
        echo -e "  Backup:     GET  /panel/api/clash-link/backup"
        echo -e "  Subscribe:  /d/{token}"
        echo -e ""
        echo -e "  Usage:"
        echo -e "  curl -X POST http://${server_ip}:${existing_port}/panel/api/clash-link/generate \\"
        echo -e "    -H 'Cookie: <your-session>' \\"
        echo -e "    -d '{\"inbound_ids\":[1],\"config_name\":\"my-node\"}'"
        echo -e "${blue}────────────────────────────────────────────${plain}"
    fi
}
